...
#+subtitle:{{{version}}}
#+date:2019-06-11 20:40
#+macro:version Version 0.0.2 {{{date}}}
* Introduction
...
- SQLite Select ::
     ...
     query data from a single table using [[http://www.sqlitetutorial.net/sqlite-select/][=SELECT=]] statement.

** Sorting rows
- SQLite Order By ::

     sort the result set in ascending or descending order using [[http://www.sqlitetutorial.net/sqlite-order-by/][=ORDER BY=]]
...
- SQLite Where ::
                  ...
                  filter rows of a result set using various conditions: [[http://www.sqlitetutorial.net/sqlite-where/][=WHERE=]]

- SQLite Limit ::

                  constrain the number of rows that you want to return.  The
                  [[http://www.sqlitetutorial.net/sqlite-limit/][=LIMIT=]] clause helps you get the necessary data returned by a
                  query.

- SQLite BETWEEN ::

                    test whether a value is in a range of values: [[http://www.sqlitetutorial.net/sqlite-between/][=BETWEEN=]]

- SQLite IN ::

               check if a value matches any value in a list of value or
               subquery: [[http://www.sqlitetutorial.net/sqlite-in/][=IN=]]

- SQLite Like  ::

                  query data based on pattern matching using wildcard
                  characters: percent sign (=%=) and underscore (=_=): [[http://www.sqlitetutorial.net/sqlite-like/][=LIKE=]]

- SQLite Glob ::

                 determine whether a string matches a specific UNIX-pattern:
                 [[http://www.sqlitetutorial.net/sqlite-glob/][=GLOB=]]
