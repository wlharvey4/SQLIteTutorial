...
#+subtitle:{{{version}}}
#+date:2019-06-15 07:40
#+macro:version Version 0.0.14 {{{date}}}
* Introduction
...
#+cindex:Node.js @command{sqlite3} module
...
#+caption:Open and close an in-memory SQLite database
#+name:connect.js
...
#+caption:Open, query, and close the Chinook database
#+name:chinook.js
...
The ~err~ argument stores the error detail in case there was an error occurred
...
#+caption:Query a playlist by its ~id~
#+name:get.js
...
#+caption:Query Data using the ~each()~ method
#+name:each.js
...
#+caption:Running Queries in ~serialize~ mode
#+name:serialize.js
...
Because the ~run()~ method returns a =Database= object so that we could chain
the method calls.
...
all the three statements may execute in parallel which would cause an error.
...
#+caption:Run function calls in parallel using ~parallelize~ mode
#+name:parallelize.js
...
#+caption:Create the ~sample.db~ database and =langs= Table
#+name:sample.js
...
calls a callback afterwards.

If an error occurred, you can find the detailed information in the =err=
argument of the callback function.

In case the statement is executed successfully, the ~this~ object of the
callback function will contain two properties:
...
#+caption:Insert a row into the =langs= table
#+name:insert.js
...
#+caption:Insert multiple rows into the =langs= table
#+name:insert-many.js
...
#+caption:How to update a row in the =langs= table
#+name:update.js
...
#+caption:How to delete a row from the =langs= table
#+name:delete.js
